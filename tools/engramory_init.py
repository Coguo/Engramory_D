...
Usage:
    ...
    python tools/engramory_init.py home                                  # global store (host-agnostic)
...
`home` creates the **global store** (`~/.engramory/` by default) that holds the
cross-project, cross-agent four-type memory (user profile, cross-project feedback,
...
def _init_home(args, source_root):
    # Global store: a host-agnostic, cross-project, cross-agent memory folder. It holds the
    # four-type memory that belongs to no single project (user profile, cross-project
    # feedback/work habits, cross-project references) and is shared by all agents via their
    # own always-loaded rules. It creates no rules file, no gitignore, no skill — each host
    # picks it up on its own (Claude Code: paste the CLAUDE.md snippet into ~/.claude/CLAUDE.md;
    # Codex/OpenClaw: their injected rules block already references ~/.engramory/; reader hosts:
    # engramory_init.py <host>-reader --memory-root ~/.engramory).
    if args.install_skill:
        # A global store has no host skill directory; copying the skill would silently do
        # nothing useful. Refuse before any side effect.
        raise SystemExit(
            f"home: --install-skill is not supported -- the global store is host-agnostic and has "
            f"no skill directory. It is shared via each host's own rules (Claude Code: paste the "
            f"CLAUDE.md snippet into ~/.claude/CLAUDE.md; a reader: `engramory_init.py "
            f"<host>-reader --memory-root ~/.engramory`). Re-run without --install-skill.")

    _require_sources(source_root, False)

    memory_root = Path(args.memory_root).expanduser()
    if args.memory_root == _DEFAULT_MEMORY_ROOT:
        # Not explicitly given -> the global store lives in the home directory.
    ...
    print("Engramory global store ready")
    print(f"memory root: {display}")
    print(f"- {result}")
    print("Loaded every session, all projects. Four-type cross-project memory "
          "(user profile / cross-project feedback / cross-project references) goes here; "
          "project-specific memory stays in each project's own store.")
    return 0


def init_host(args, host):
    if host == "home":
        return _init_home(args, _repo_root())
...
def build_parser():
    ...
    parser.add_argument("host", nargs="?", default="codex",
                        choices=tuple(HOST_CONFIG) + ("home",),
                        help="host to initialize: 'home' (global store), write hosts (codex, "
                             "openclaw) or a read-only reader host '<host>-reader' "
                             "(codex-reader, claude-reader, cursor-reader, kiro-reader, "
                             "cline-reader, etc.)")
    parser.add_argument("--project-root", default=None,
                        help="project/workspace root (default: '.'; openclaw defaults to ~/.openclaw/workspace)")
    parser.add_argument(
        "--memory-root",
        default=_DEFAULT_MEMORY_ROOT,
        help="memory store path; relative paths are resolved under --project-root. For a "
             ...
             "e.g. Claude Code's memory dir ~/.claude/projects/<project>/memory. For 'home' "
             "the default is ~/.engramory (the global store)",
    )
