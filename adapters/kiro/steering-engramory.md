...
You have a curated, file-based memory under `memory/` (index: `MEMORY.md`,
...
- **At the start of a task**, scan the index below (one line per memory) and open only
  the detail files (`memory/<Type>/<slug>.md`) whose hooks look relevant. Treat
...
- **When you learn something durable** worth a future session: confirm it isn't already
  ...
  *value*; search the index and **update an existing note** rather than duplicate;
  otherwise write one atomic markdown file (one fact) in its **type subfolder** under
  `memory/` — `feedback/` · `project/` · `reference/` — with
  frontmatter `name` / `description` (a sharp one-line hook) / `type` / `created` +
  `updated` (`YYYY-MM-DD`). A `feedback` or `project` note must also carry a **`Why:`**
  line and a **`How to apply:`** line. Add one pointer line to `MEMORY.md`, carrying the
  subfolder: `- [<title>](project/<slug>.md) — <one-line hook>`. **Delete** notes that
  turn out wrong. A project store never creates a `user` note — who the user is is
  cross-project, so it lives only in the global store.
...
- Keep `MEMORY.md` small (soft 150 lines / 20 KB, hard 200 lines / 25 KB). If a write
  ...
  run `python tools/engramory_check.py memory/MEMORY.md` (path relative to
