...
# --- engramory_init (home: host-agnostic global store) ---

def test_init_home_creates_global_store(tmp_path):
    # an explicit --memory-root -> creates the store with a MEMORY.md from the template,
    # prints a global-store verdict, and never touches a gitignore/rules file.
...
def test_init_home_defaults_to_home_engramory(tmp_path):
    # without --memory-root -> Path.home()/".engramory". Override HOME+USERPROFILE so the
    # test never writes to the real home directory.
    fake = tmp_path / "home"
    rc, out = _run(INIT, "home", env={"HOME": str(fake), "USERPROFILE": str(fake)})
    assert rc == 0 and "global store ready" in out
    assert (fake / ".engramory" / "MEMORY.md").is_file()
    assert "~/.engramory" in out  # shown with the ~/ form


def test_init_home_keeps_existing_memory_index(tmp_path):
    # idempotent: a second run must keep an existing MEMORY.md, not overwrite it.
    root = tmp_path / "global"
    root.mkdir(parents=True)
    (root / "MEMORY.md").write_text("# Custom Global Index\n", encoding="utf-8")
    rc, out = _run(INIT, "home", "--memory-root", str(root))
    assert rc == 0 and "kept existing MEMORY.md" in out
    assert (root / "MEMORY.md").read_text(encoding="utf-8") == "# Custom Global Index\n"


def test_init_home_refuses_install_skill(tmp_path):
    # the global store is host-agnostic (no skill dir); --install-skill must be refused
    # before any side effect.
    root = tmp_path / "global"
    p = subprocess.run([sys.executable, INIT, "home", "--memory-root", str(root),
                        "--install-skill"], capture_output=True, text=True)
    assert p.returncode != 0 and "install-skill" in (p.stdout + p.stderr)
    assert not (root / "MEMORY.md").exists()  # nothing written (refused before side effects)


def test_init_home_ascii_console_does_not_crash(tmp_path):
    # the global-store verdict lines must survive a strict ascii/OEM console.
    rc, out = _run(INIT, "home", "--memory-root", str(tmp_path / "g"),
