...
def test_ignore_path_exempts_growth(tmp_path):
    ...
    guarded even when the edit grows it past the cap."""
    idx = _write(tmp_path / "MEMORY.md", lines=40)
    rc, out = _run(idx, _w(idx, "\n".join(["L"] * 250)),
                   env={"ENGRAMORY_INDEX_IGNORE": str(idx)})
    assert rc == 0 and _decision(out) == "(silent)"


def test_ignore_basename_exempts_any_dir(tmp_path):
    """A bare basename in ENGRAMORY_INDEX_IGNORE exempts a file with that name in ANY
    directory — e.g. a repo's templates/MEMORY.md."""
    idx = _write(tmp_path / "MEMORY.md", lines=40)
    rc, out = _run(idx, _w(idx, "\n".join(["L"] * 250)),
                   env={"ENGRAMORY_INDEX_IGNORE": "MEMORY.md"})
    assert rc == 0 and _decision(out) == "(silent)"


def test_ignore_does_not_exempt_other_files(tmp_path):
    """Ignoring one file must leave the rest guarded — a non-ignored MEMORY.md still
    grows-past-cap DENIES even when the ignore var is set for a different path."""
    idx = _write(tmp_path / "MEMORY.md", lines=40)
    other = tmp_path / "OTHER.md"
    rc, out = _run(idx, _w(idx, "\n".join(["L"] * 250)),
                   env={"ENGRAMORY_INDEX_IGNORE": str(other)})
    assert rc == 0 and _decision(out) == "deny"


def test_ignore_comma_separated_list(tmp_path):
    """Multiple comma-separated ignore entries all exempt; a non-listed one is guarded."""
    idx = _write(tmp_path / "MEMORY.md", lines=40)
    one = tmp_path / "one.md"
    two = tmp_path / "two.md"
    rc, out = _run(idx, _w(idx, "\n".join(["L"] * 250)),
                   env={"ENGRAMORY_INDEX_IGNORE": f"{one},{two}"})
    assert rc == 0 and _decision(out) == "deny"  # idx not in the ignore list
    # ignore idx + another, now idx is exempt:
    rc, out = _run(idx, _w(idx, "\n".join(["L"] * 250)),
                   env={"ENGRAMORY_INDEX_IGNORE": f"{one},{idx}"})
    assert rc == 0 and _decision(out) == "(silent)"


def test_ignore_full_path_does_not_exempt_same_basename_elsewhere(tmp_path):
    # Regression: ignoring templates/MEMORY.md by full path must NOT exempt the real
    # memory/MEMORY.md index just because both files share the basename "MEMORY.md".
    idx = _write(tmp_path / "MEMORY.md", lines=40)
    other = tmp_path / "templates" / "MEMORY.md"
    rc, out = _run(idx, _w(idx, "\n".join(["L"] * 250)),
                   env={"ENGRAMORY_INDEX_IGNORE": str(other)})
    assert rc == 0 and _decision(out) == "deny"


def test_ignore_empty_value_is_noop(tmp_path):
    """An empty/whitespace ENGRAMORY_INDEX_IGNORE must behave as unset (guard works)."""
    idx = _write(tmp_path / "MEMORY.md", lines=40)
    rc, out = _run(idx, _w(idx, "\n".join(["L"] * 250)),
                   env={"ENGRAMORY_INDEX_IGNORE": " , ,"})
    assert rc == 0 and _decision(out) == "deny"
