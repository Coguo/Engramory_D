...
Config via environment variables (all optional):
  ...
  ENGRAMORY_INDEX_IGNORE  comma-separated EXEMPTIONS from the guard. A full path
                       ...
                       indexes too). Use it when some MEMORY.md is NOT a real index,
                       while the filename-based default keeps guarding the rest.
"""
import json
import os
import sys
...
def main():
    ...
    ignore = [p.strip() for p in
              os.environ.get("ENGRAMORY_INDEX_IGNORE", "").split(",") if p.strip()]
    ...
    if ignore:
        ...
        for ign in ignore:
            if os.sep in ign or (os.altsep and os.altsep in ign):
                if _key(ign) == fk:
                    _allow_silently()
            elif os.path.basename(ign).lower() == base_lower:
                _allow_silently()
