...
### 全局记忆库(可选,多 agent / 跨项目)

跨项目记忆——用户是谁、跨项目工作习惯、跨项目资源指针——有一个**宿主无关的全局库**安放,这样它被所有 agent 共享,而不是每个项目库复制一份:
...
全局库和项目库一样是纯四类型 markdown 文件夹,但它不属于任何单个项目,且每个会话都读取。每个宿主通过自己的常驻规则接入:**Claude Code** —— 把 [`templates/claude-engramory-snippet.md`](templates/claude-engramory-snippet.md) 贴进 `~/.claude/CLAUDE.md`;**Codex/OpenClaw** —— 它们注入的规则块已经引用 `~/.engramory/`;**只读宿主** —— `python tools/engramory_init.py <host>-reader --memory-root ~/.engramory` 只读召回。索引上限对每个 `MEMORY.md` 各自独立计数,全局索引应保持天然很小。
...
- **作用域 / 多项目** —— 没有 `scope` / `project_id`;单一扁平 slug 命名空间,跨项目 / 跨 agent 共用一个库会撞 slug、串项目。一个库级 manifest(协议版本 + 作用域 + 宿主配置)是规划中的第一步——**还没做**。(**全局库**用物理分库的方式提供了跨项目*层级*——跨项目事实放 `~/.engramory/`,项目事实放项目库,互不挤一个索引;但它没有在每条记忆的 schema 上增加 scope 字段。)
- **并发** —— 假设单写者 / 串行写入(无锁)。
