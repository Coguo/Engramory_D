...
Memory lives in **two tiers**, both directories the **user can see, open, and
audit**. Human-readability is the whole point — never hide a store somewhere the
user will not look.

- **Global tier** — `<GLOBAL_MEMORY_ROOT>` (default `~/.engramory/`, configurable).
  Holds the **cross-project** four-type memory: who the user is (`user`),
  cross-project work habits / general procedures (`feedback`), cross-project resource
  pointers (`reference`). It belongs to no single project and is shared by every agent
  that reads it. Skip it if the directory doesn't exist.
- **Project tier** — `<MEMORY_ROOT>`, commonly a `memory/` directory at the project
  root, created once per project (e.g. by a `memory-init` scaffold). Holds **this
  project's** state and decisions (`project`), project-specific habits (`feedback`)
  and pointers (`reference`). It never holds `user` notes — who the user is is always
  cross-project, so that type belongs only in the global tier. On a host with native
  auto-memory (e.g. Claude Code), `<MEMORY_ROOT>` may be that host's memory directory
  and Engramory layers its conventions on top.

Rules that apply to **both** tiers:
- Both roots MUST be configurable; never hard-code an absolute path in the skill
  (the global default `~/.engramory/` is a convention, not a requirement).
- If a tier's root lives inside a git repository, it MUST be git-ignored — memories
  routinely contain machine-local, sensitive but non-credential detail (server IPs,
...
tier; a project store never creates a `user` note. When unsure, prefer the project tier —
misplacing a fact in the global tier is costlier because it loads every session.
...
`user` notes live only in the **global tier**; a project store never creates one
(who the user is is always cross-project).
...
1. At the start of a task, read **both** `MEMORY.md` indexes: the global tier's
   (`<GLOBAL_MEMORY_ROOT>/MEMORY.md`, if the directory exists) and the project tier's
   (`<MEMORY_ROOT>/MEMORY.md`, if the project store exists). (On a host with native
   auto-memory, its project index is already loaded — just apply this discipline to it
   and read the global one too if present.)
2. Scan the one-line descriptions in **both** indexes. Open only the detail files whose
   hooks look relevant to the task at hand. Do not bulk-read everything.
...
0. **Pick the tier — which store does this belong to?** Cross-project / about the
   user → the **global tier** (`<GLOBAL_MEMORY_ROOT>`); about this project → the
   **project tier** (`<MEMORY_ROOT>`). A `user` note goes only to the global tier; a
   project store never creates one. When unsure, prefer the project tier (the
   global index loads every session, so a wrong placement costs more).
...
2. **Dedup — does a memory already cover this?** Read the **chosen tier's** index
   first; if an existing file covers the same ground, **update that file** (and bump
   `updated:`) rather than create a near-duplicate. If the fact might also live in the
   other tier, glance at its index too before deciding.
...
Each index loads in full every session, so its size is a recurring cost — and many
hosts only load the first ~200 lines / ~25 KB of each, meaning anything past that
silently stops being recalled. Keep **each** `MEMORY.md` bounded; the two tiers count
**independently** (a bloated global index does not excuse a bloated project index, and
vice versa).

**Every time you are about to modify a `MEMORY.md`, check BOTH its line count and
...
matches the index by filename (`MEMORY.md`), so it guards **both** tiers' indexes; it
blocks edits that would *grow* an index past the caps, but always allows *shrinking*
edits so you can compact incrementally (e.g. 210 → 205 → 198). It only injects nudges —
it never auto-approves. The soft warnings and the compaction judgment are your job
either way.
...
- A host with native auto-memory (Claude Code) already auto-loads `MEMORY.md` and
  ...
  memory directory as `<MEMORY_ROOT>`, or, if you prefer a store you fully control
  and want shared across agents, use a dedicated `memory/` directory at the project
  root (scaffolded once, e.g. by a `memory-init` skill).
