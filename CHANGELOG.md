...
This is the merge of two branches cut from the same point, which reached that conclusion
independently: 0.6.2 (below) repaired the same `engramory_init.py` source check on its way
to cross-platform support. Its platform work is kept as-is; where the two overlapped, the
resolutions here supersede it — so this entry lists what 0.6.2 does *not* already cover.
...
Added
- **`home` host in `engramory_init.py`.** Creates the global store (`~/.engramory/` by
  default; `--memory-root` to relocate). It is a plain four-type markdown folder like any
...
- `rules-snippet.md` rewritten for the two tiers: recall reads **both** indexes; save picks
  ...
  `codex`/`openclaw` now reference `~/.engramory/` automatically (the snippet's literal
  global path, `<MEMORY_ROOT>` still replaced by init).
